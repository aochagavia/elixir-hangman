defmodule Game.State do
  ...
  @type t :: %Game.State{word: String.t(), guesses: [integer], failure_count: integer}
  @type guess_result :: {:good_guess, [integer]} | :wrong_guess | :existing_guess
  @type game_outcome :: :player_wins | :player_loses | :unknown

  @spec with_word(String.t()) :: Game.State.t()
  def with_word(word) do
    %Game.State{word: word}
  end

  @spec update(Game.State.t(), integer) :: {Game.State.guess_result(), Game.State.t()}
  ...
  @spec game_outcome(Game.State.t()) :: Game.State.game_outcome()
